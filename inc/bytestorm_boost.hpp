...
inline void update_clients_changed()
{
    for (ClientVector::iterator b = clients.begin(), e = clients.end(); b != e; ++b) (*b)->set_clients_changed();
}

template <typename ServerType>
inline void handle_accept(ServerType client, const boost::system::error_code &err, HandlerBase<ServerType> *neth)
{
    client->start();
    neth->set(&client);
